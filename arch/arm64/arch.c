...
void arch_init(void) {
  ...
  /* flush the release of the lock, since the secondary cpus are running without
   * cache on */
...
/* switch to user mode, set the user stack pointer to user_stack_top, put the
 * svc stack pointer to the top of the kernel stack */
void arch_enter_uspace(vaddr_t entry_point, vaddr_t user_stack_top) {
  ...
  asm volatile("mov    sp, %[kstack];"
               "msr    sp_el0, %[ustack];"
               "msr    elr_el1, %[entry];"
               "msr    spsr_el1, %[spsr];"
               "eret;"
               :
               : [ustack] "r"(user_stack_top), [kstack] "r"(kernel_stack_top),
