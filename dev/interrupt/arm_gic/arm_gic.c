...
#define DEFINE_GIC_SHADOW_REG(name, bit_per_reg, init_val, init_from)          \
  uint32_t(name)[GIC_REG_COUNT(bit_per_reg)] = {                               \
      [(init_from / bit_per_reg)...(GIC_REG_COUNT(bit_per_reg) - 1)] =         \
          (init_val)}
...
static int arm_gic_max_cpu(void) { return (GICREG(0, GICD_TYPER) >> 5) & 0x7; }
...
static status_t arm_gic_set_secure_locked(u_int irq, bool secure) {
  ...
  LTRACEF("irq %d, secure %d, GICD_IGROUP%d = %x\n", irq, secure, reg,
          GICREG(0, GICD_IGROUPR(reg)));
#endif
  return NO_ERROR;
}

static status_t arm_gic_set_target_locked(u_int irq, u_int cpu_mask,
                                          u_int enable_mask) {
  u_int reg = irq / 4;
  u_int shift = 8 * (irq % 4);
  u_int old_val;
  ...
  LTRACEF("irq %i, GICD_ITARGETSR%d %x => %x (got %x)\n", irq, reg, old_val,
          new_val, GICREG(0, GICD_ITARGETSR(reg)));
...
status_t arm_gic_sgi(u_int irq, u_int flags, u_int cpu_mask) {
  u_int val = ((flags & ARM_GIC_SGI_FLAG_TARGET_FILTER_MASK) << 24) |
              ((cpu_mask & 0xff) << 16) |
...
static enum handler_return __platform_irq(struct iframe *frame) {
  ...
  LTRACEF_LEVEL(2, "iar 0x%x cpu %u currthread %p vector %d pc 0x%lx\n", iar,
                cpu, get_current_thread(), vector, (uintptr_t)IFRAME_PC(frame));
...
static u_int current_fiq[8] = {0x3ff, 0x3ff, 0x3ff, 0x3ff,
                               0x3ff, 0x3ff, 0x3ff, 0x3ff};

static bool update_fiq_targets(u_int cpu, bool enable, u_int triggered_fiq,
                               bool resume_gicd) {
  u_int i, j;
  u_long mask;
  u_int fiq;
